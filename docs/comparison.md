...
| ISOBMFF | validated, hard binding included | **MP4, MOV, AVIF and HEIC** read and verified, hard binding included, each held by a fixture (SPEC-026, SPEC-027). Fragmented files, Merkle trees, the `subset`/`length`/`version`/`flags` filters and nested exclusion paths are each refused by name | a fragmented fixture, then the Merkle spec |
