...
| A claim whose `signature` URI is `self#jumbf=c2pa/<manifest>/c2pa.signature`, without the leading slash (Microsoft Bing Image Creator, nine files from Commons, step 141) | the signature is found | `claimSignature.missing`: a path without a slash is read as relative to the manifest | SPEC-042 |
