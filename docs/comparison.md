...
| ISOBMFF | validated, hard binding included | **MP4 and AVIF** read and verified, hard binding included (SPEC-026, SPEC-027). MOV verifies but is not in the fixtures, so no test holds it; HEIC is unmeasured. Fragmented files, Merkle trees, the `subset`/`length`/`version`/`flags` filters and nested exclusion paths are each refused by name | a MOV fixture, then a fragmented one, then a spec |
