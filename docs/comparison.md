...
| A top-level `trust.allowed_list` is refused (exit 2, a message naming `trust.anchors[].allowed_list`); `c2patool` 0.28.0 drops it without a word, 0.27.22 honoured it | the same settings file would otherwise mean `Trusted` here and `Valid` there, silently | SPEC-031 AC4 |
| An unknown key inside a `trust.anchors` entry, or an `allowed_list` on a `"tsa"` entry, is refused; `c2patool` 0.28.0 ignores both | settings are whole or absent (SPEC-014 AC7); §14.4.3 keeps the allowed list away from time-stamps | SPEC-031 AC5, AC6 |
...
| A claim whose URIs are written `self#jumbf=c2pa/<manifest>/…`, without the leading slash, is read as the spec says: relative to the current manifest, where no `c2pa` box exists, so the signature is `claimSignature.missing`. `c2pa-rs` reads `c2pa/…` as `/c2pa/…` (`to_normalized_uri`) and finds it | C2PA 2.4 §8.4.2.1: a `self#jumbf` URI is relative to the store only when it starts with `/`, otherwise to the current manifest; §10.2.2: the `signature` field shall hold an *absolute* URI; §15.7: a signature URI that cannot be resolved is `claimSignature.missing`. The spec's own examples wrote this form from 1.0 to 2.1, and Microsoft Bing Image Creator follows them (claim v1, nine files from Commons, step 141). Those files also carry `c2pa.hash.boxes`, a hard binding refused by name (`PRED-CONT-006`), so reading the URI would not make them `Valid` | a decision, step 143 |
