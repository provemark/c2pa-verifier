...
| ISOBMFF | validated, hard binding included | **MP4, MOV, AVIF and HEIC** read and verified, hard binding included, each held by a fixture (SPEC-026, SPEC-027). **Fragmented streams verified too** (SPEC-028): the init segment against `initHash` and every fragment against the Merkle root. The `subset`/`length`/`version`/`flags` filters, nested exclusion paths and an assertion with more than one `merkle` map are each refused by name | a stream with several renditions |
