...
| A JPEG whose first APP11 piece carries packet sequence number Z = 0 (Microsoft Bing Image Creator writes every store this way) | read: `c2pa-rs` does not check the first piece's Z | refused (`general.error`, SPEC-001 AC3); nine Bing files from Commons, step 141 | a spec, once ISO/IEC 19566-5 has been read on the first Z |
