...
| SPEC-023 | The published package: every top-level path classified, what the dist must and must not hold, the shipped markdown self-contained, the disclosure carried along, and the archive proven to run unpacked and alone | draft 2026-09-22, awaiting approval; both blocking questions decided the same day — a test in this spec's group may invoke `git`, and `specs/`/`notes/` ship with the package |
