...
| SPEC-022 | Update manifests (`c2um`): the box read (SPEC-005 AC13 amended for `c2um` only), §11.2.3's four rules with `manifest.update.invalid` / `.wrongParents` and §15.11's `manifest.multipleParents`, the binding found through the `parentOf` chain (§15.12) and its stale exclusion adjusted to the store's current range (§15.12.1.1 — measured: 18874 against 43607), an empty `claim_generator_info` counted as absent; closes the last `_MULTI` file | approved 2026-09-22 |
