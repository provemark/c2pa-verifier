...
/*
 * Build the fixtures of SPEC-040 (step 137a): the signed PNG fixture with its actions entry in
 * created_assertions rewritten (absolute to its own manifest, to another manifest, to no manifest
 * label), the claim re-signed under a throwaway root, and the COSE padded so that the store keeps
 * its length — the data hash then needs no rebinding. Both c2patool versions judge every variant.
 *
 * Usage: php bin/make-spec040-variants.php <c2patool-0.28.0> <c2patool-0.27.22>
 * Writes:
 *   tests/Fixtures/outside-manifest/<variant>.png, probe-root.pem, probe-root.settings.json
 *   tests/Fixtures/c2patool/outside-manifest/<variant>--<version>.json (or .error.txt)
 * Tooling.
 */
...
if (! is_file($new) || ! is_file($old) || $version($new) !== 'c2patool 0.28.0' || $version($old) !== 'c2patool 0.27.22') {
    fwrite(STDERR, "usage: php bin/make-spec040-variants.php <c2patool-0.28.0> <c2patool-0.27.22>\n");
    exit(1);
}
$keys = sys_get_temp_dir().'/c2pa-spec040-'.bin2hex(random_bytes(6));
if (! mkdir($keys, 0o700)) {
...
$out = $root.'/tests/Fixtures/outside-manifest';
$oracles = $root.'/tests/Fixtures/c2patool/outside-manifest';
...
$relative = 'self#jumbf=c2pa.assertions/c2pa.actions.v2';
$variants = [
    'own-absolute' => "self#jumbf=/c2pa/{$label}/c2pa.assertions/c2pa.actions.v2",
    'other-manifest' => 'self#jumbf=/c2pa/urn:c2pa:00000000-0000-4000-8000-000000000000/c2pa.assertions/c2pa.actions.v2',
    'no-manifest-label' => 'self#jumbf=/c2pa/',
];
...
foreach ($variants as $name => $url) {
    ...
    $manifestBox = $root2->superboxes()[0];
    $claimSuper = $manifestBox->child('c2pa.claim.v2') ?? throw new RuntimeException('no claim box');
    $sigSuper = $manifestBox->child('c2pa.signature') ?? throw new RuntimeException('no signature box');
    $claimBytes = $claimSuper->contentBoxes()[0]->data;
    ...
    file_put_contents("{$out}/{$name}.png", pngWithStore($png, $bytes));
}
