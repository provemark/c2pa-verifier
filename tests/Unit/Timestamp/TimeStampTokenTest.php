...
function spec016Five(): array
{
    ...
    return [
        ...
        'public-testfiles/adobe-20220124-C.jpg' => $digicert + ['header' => 'sigTst', 'imprint' => '0432594d', 'serialHex' => '86B091F8B163977B2B98CDC2A02D7B0F', 'genTime' => '2023-01-24T14:48:56+00:00', 'nonceHex' => '90C0FF57DE1168F8', 'eContentLength' => 114, 'eContentHead' => '3070', 'signerCn' => 'DigiCert Timestamp 2022'],
...
function spec016HeaderValue(string $relative): string
{
    ...
    if (! is_array($header) || ! isset($header['tstTokens'][0]['val']) || ! $header['tstTokens'][0]['val'] instanceof CborBytes) {
        throw new RuntimeException("{$relative} carries no timestamp header");
    }

    return $header['tstTokens'][0]['val']->bytes;
}
...
function spec016Splice(string $bytes, int $at, int $oldLength, string $new, ?int $inside = null): string
{
    ...
    foreach (array_reverse($path) as $p) {
        ...
        $header = chr($element['tag']).spec016Length($element['length'] + $delta);
...
describe('SPEC-016 AC3: the five tokens\' TSTInfo, field by field as openssl ts prints it', function (): void {
    foreach (spec016Five() as $file => $expected) {
        test($file, function () use ($file, $expected): void {
...
describe('SPEC-016 AC4: the SignedData and the one SignerInfo', function (): void {
    foreach (spec016Five() as $file => $expected) {
        test($file, function () use ($file, $expected): void {
            $token = TimeStampToken::fromHeaderValue(spec016HeaderValue($file));
            $sd = $token->signedData;
            ...
            expect($sd->version)->toBe(3)
                ...
                ->and(bin2hex(substr($sd->eContent, 0, strlen($expected['eContentHead']) / 2)))->toBe($expected['eContentHead'])
                ->and($sd->digestAlgorithms)->toContain($expected['digestAlg'])
                ->and(count($sd->certificates))->toBe(3);
            $leaf = Certificate::fromDer($sd->certificates[0]);
            foreach ($sd->certificates as $der) {
                Certificate::fromDer($der); // every one is a certificate ext-openssl accepts
            }
            // the sid: issuerAndSerialNumber, equal to the leaf's issuer Name (TBSCertificate's fourth element) and serial
            $tbs = (new DerReader)->read($sd->certificates[0])->child(0);
...
describe('SPEC-016 AC5: the cut is right — the re-tagged signed attributes verify', function (): void {
    foreach (spec016Five() as $file => $expected) {
        test($file, function () use ($file, $expected): void {
            $token = TimeStampToken::fromHeaderValue(spec016HeaderValue($file));
            $si = $token->signedData->signerInfo;
            $key = openssl_pkey_get_public(Certificate::pem($token->signedData->certificates[0]));
...
describe('SPEC-016 AC6: both wrappers, either header', function (): void {
    test('a TimeStampResp (sigTst) and a bare ContentInfo (sigTst2) parse, and so do each other\'s shape', function (): void {
        $response = spec016C();
        $bare = spec016HeaderValue('c2pa-rs/C_with_CAWG_data.jpg');
        expect(strlen($response))->toBe(5951)->and(bin2hex(substr($response, 0, 9)))->toBe('3082173b300302010')
    ...
    test('status 1 (grantedWithMods) is accepted; status 2 with a statusString is refused naming both; a granted response without a token is refused', function (): void {
        $response = spec016C();
    ...
    test('a ContentInfo whose OID is not signedData is refused naming the OID', function (): void {
        $bare = spec016HeaderValue('c2pa-rs/C_with_CAWG_data.jpg');
...
describe('SPEC-016 AC7: the token\'s own rules are enforced, one refusal each', function (): void {
    /*
    ...
    test('the unpatched token parses (every helper below changes only what it claims)', function () use ($c): void {
        expect(TimeStampToken::fromHeaderValue($c())->tstInfo->version)->toBe(1);
    });

    test('eContentType that is not id-ct-TSTInfo', function () use ($c): void {
        $bytes = $c();
    ...
    test('two SignerInfos', function () use ($c): void {
        $bytes = $c();
    ...
    test('zero SignerInfos', function () use ($c): void {
        $bytes = $c();
        $set = spec016SignerInfoSet($bytes);
    ...
    test('TSTInfo version 2', function () use ($c): void {
        $bytes = $c();
    ...
    test('a messageImprint of 31 bytes for sha256', function () use ($c): void {
        $bytes = $c();
    ...
    test('a SignerInfo without signedAttrs', function () use ($c): void {
        $bytes = $c();
    ...
    test('signedAttrs without messageDigest', function () use ($c): void {
        $bytes = $c();
    ...
    test('a contentType attribute that is not the eContentType', function () use ($c): void {
        $bytes = $c();
    ...
    test('a certificates choice that is not certificate ([1] extraCert)', function () use ($c): void {
        $bytes = $c();
    ...
    test('no certificates at all', function () use ($c): void {
        $bytes = $c();
    ...
    test('a TSTInfo with a critical extension', function () use ($c): void {
        $bytes = $c();
        // Extensions [3] { Extension { OID 1.2.3.4, critical TRUE, OCTET STRING "" } } appended to the TSTInfo
        $extension = "\x30\x0a"."\x06\x03\x2a\x03\x04"."\x01\x01\xff"."\x04\x00";
        $extensions = "\xa3".chr(strlen($extension) + 2)."\x30".chr(strlen($extension)).$extension;
...
describe('SPEC-016 AC8: bounded — a token is at most maxBytes, a header at most maxTokens', function (): void {
    test('a header with neither sigTst nor sigTst2 is null; one with both is refused', function (): void {
    ...
    test('the shape: tstTokens, each with a byte-string val', function (): void {
    ...
    test('nine tokens exceed maxTokens 8', function (): void {
    ...
    test('a value of maxBytes + 1 is refused before it is read (the default is 1 MiB; the test lowers it to keep the bytes small)', function (): void {
    ...
    test('the claim version is not this spec\'s rule: sigTst2 on a v1 claim and sigTst on a v2 claim pass here', function (): void {
...
describe('SPEC-016 AC9: CA_ct.jpg is the corpus\'s malformed token, and the message says why', function (): void {
    test('genTime 20240806216337Z: minute 63 at offset 87 of the TSTInfo', function (): void {
        $value = spec016HeaderValue('c2pa-rs/CA_ct.jpg');
        try {
            TimeStampToken::fromHeaderValue($value);
            $this->fail('CA_ct.jpg parsed');
        } catch (TimestampException $e) {
            expect($e->getMessage())->toContain('genTime')->toContain('63')->toContain('offset 87');
        }
...
describe('SPEC-016 AC10: every corpus token parses, and none takes the reader past its bounds', function (): void {
    test('38 timestamped files, 37 parse; one token, one SignerInfo, version 1, three certificates each', function (): void {
        $files = [];
        ...
        foreach ($files as $relative) {
            ...
            expect($token->tstInfo->version)->toBe(1, $relative)
                ->and($token->signedData->certificates)->toHaveCount(3, $relative);
