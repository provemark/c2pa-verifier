...
it('AC9: the codes are verbatim, and success is told apart', function (): void {
    ...
    foreach (StatusCode::cases() as $code) {
        if (str_starts_with($code->value, 'assertion.dataHash') || str_contains($code->value, 'HardBindings') || in_array($code->value, ['signingCredential.trusted', 'signingCredential.untrusted'], true)) {
            continue;   // SPEC-012's and SPEC-014's
        }
