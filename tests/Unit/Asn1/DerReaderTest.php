...
describe('SPEC-016 AC1: the ten tags decode, and the values are X.690\'s', function (): void {
    test('INTEGER: 0, 127, 128 (two\'s complement keeps the leading zero), and a sixteen-byte serial as a decimal string', function (): void {
    ...
    test('OBJECT IDENTIFIER: DigiCert\'s timestamp policy and id-ct-TSTInfo, dotted decimal', function (): void {
        expect(spec016Der('06 0a 60 86 48 01 86 fd 6c 07 01')->oid())->toBe('2.16.840.1.114412.7.1')
    ...
    test('NULL, BOOLEAN, OCTET STRING', function (): void {
    ...
    test('UTCTime and GeneralizedTime: the same instant three ways, fractions dropped, the century rule of RFC 5280', function (): void {
    ...
    test('SEQUENCE and SET: children with their offsets, an empty SET, and encoded() returns the input', function (): void {
    ...
    test('context-specific tags: [0] constructed holding an INTEGER, [0] primitive holding raw bytes', function (): void {
    ...
    test('a long-form length: 04 82 01 00 and 256 bytes, header 4, whole element 260', function (): void {
    ...
    test('the wrong accessor throws, naming the offset and both tags', function (): void {
        $oid = spec016Der('06 0a 60 86 48 01 86 fd 6c 07 01');
...
describe('SPEC-016 AC2: malformed DER is refused with the offset, never read past', function (): void {
    ...
    foreach ($cases as $name => [$hex, $accessor, $fragment]) {
        test($name, function () use ($hex, $accessor, $fragment): void {
            ...
            try {
                ...
                $this->fail("{$hex} was read without an exception");
            } catch (Asn1Exception $e) {
                expect($e->getMessage())->toContain($fragment);
                expect(spec016OffsetIn($e->getMessage()))->toBeGreaterThanOrEqual(0, 'the message names an offset: '.$e->getMessage());
                expect(spec016OffsetIn($e->getMessage()))->toBeLessThanOrEqual(strlen($bytes), 'the offset lies within the input: '.$e->getMessage());
            }
        });
    }

    test('nesting 33 deep exceeds maxDepth 32; 32 deep does not', function (): void {
        $nest = static function (int $depth): string {
            $bytes = "\x05\x00";
            for ($i = 0; $i < $depth; $i++) {
                $bytes = "\x30".chr(strlen($bytes)).$bytes;
            }
    ...
    test('65 537 elements in one SEQUENCE exceed maxElements 65 536', function (): void {
    ...
    test('an input of maxBytes + 1 is refused before reading', function (): void {
