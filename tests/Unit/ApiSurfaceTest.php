...
it('AC1: the recorded surface is exactly what the contract classes expose today', function (): void {
    ...
    expect($live)->toBe(spec025Recorded())
        ->and($live)->toHaveCount(124);   // 91, then two bmffHash codes, then SPEC-028's tenth class, then SPEC-030's four ocsp codes, then SPEC-031's eleventh class (10) and two members of TrustSettings, then SPEC-032's code, then SPEC-033's two, then SPEC-035's six, then SPEC-036's one, then SPEC-037's one, then SPEC-038's two
})->group('SPEC-025');
