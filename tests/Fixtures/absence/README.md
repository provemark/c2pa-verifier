...
| `hash-data-gathered` | nothing absent: `hash.data` moved to `gathered_assertions`, actions to `created` | `Valid` / `Trusted` — placement is attribution, not validated (c2pa-rs 0.90); **0.28.0: `Error: claim missing hard binding`** | `Valid` / `Trusted` until step 118; now `Invalid`, `claim.hardBindings.missing` (SPEC-013 AC19, C2PA 2.4 §10.2.2) |
